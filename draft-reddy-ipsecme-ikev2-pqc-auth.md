...
{{!RFC7427}} defines the SIGNATURE_HASH_ALGORITHMS notification, where each side of the IKE negotiation lists its supported hash algorithms, assuming that signature schemes involve a hashing phase followed by a signature phase. The data signed to prove the identity of the initiator and responder (as discussed in Section 2.15 of {{!RFC7427}}) typically fits within the memory constraints of the devices involved in the IKEv2 exchange, consisting of nonces, SPIs, and the initial exchange messages, which are manageable in size. To signal that no pre-hashing is required for either ML-DSA or SLH-DSA at the IKEv2 level, the 'Identity' (5) value defined in {{!RFC8420}} can be set in the SIGNATURE_HASH_ALGORITHMS notification. When hashing is applied, the digest must be generated using an approved hash function or XOF (e.g., from [FIPS180] or [FIPS202]) that provides at least 𝜆 bits of classical security strength against collision and second preimage attacks, as per Table 4 of [FIPS202].
